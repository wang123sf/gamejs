...
exports.init = function() {
   ...
   var canvas = gamejs.display._getCanvas();
   var lastPos = [];

   // if the canvas is zoomed via css, then the mouse event pixel positions
   // must be scaled to fit the new canvas size
   var getNormalizedMousePosition = function(event) {
      var boundingRect = canvas.getBoundingClientRect();
      var canvasOffset = [boundingRect.left, boundingRect.top];
      var factor = [
         (canvas.width || parseInt(canvas.style.width, 10)) / canvas.clientWidth,
         (canvas.height || parseInt(canvas.style.height, 10)) / canvas.clientHeight
      ];
      return [
         (event.clientX - canvasOffset[0]) * factor[0], 
         (event.clientY - canvasOffset[1]) * factor[1]
      ];
   };
   ...
   function onMouseDown (ev) {
      QUEUE.push({
         ...
         'pos': getNormalizedMousePosition(ev),
         'button': ev.button,
   ...
   function onMouseUp (ev) {
      var boundingRect = gamejs.display._getCanvas().getBoundingClientRect();
      var canvasOffset = [boundingRect.left, boundingRect.top];
      QUEUE.push({
   ...
   function onMouseMove (ev) {
      var currentPos = getNormalizedMousePosition(ev);
      var relativePos = [];
      if (getGrab()) {
         relativePos = [
            ev.movementX,
   ...
   // for grab/mouselock
   window.navigator.pointer = window.navigator.pointer || window.navigator.webkitPointer;
};

/**
 * Your gamejs application will share the mouse and keyboard with other
 * elements on the same page. If your program sets the event grab to `True`,
 * gamejs will lock all input into your program.
 * It is best to not always grab the input, since it prevents the user from
 * doing other things.
 *
 * @param {Boolean} newValue
 * @return {Boolean} false if grabbing is not supported by this browser.
 */
exports.setGrab = function(newValue) {
   if (!window.navigator.pointer) {
      return false
   }
   window.navigator.pointer.lock(gamejs.display._getCanvas(), function() {
   });
   // FIXME this is not true: the success is async
   return true;
};
/**
 * @returns {Boolean} true when the input events are grabbed for
 * this application.
 */
var getGrab = exports.getGrab = function() {
   if (!window.navigator.pointer) {
      return false;
   }
   return window.navigator.pointer.isLocked();
};
